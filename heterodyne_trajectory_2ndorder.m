function [m2x, m2y, m2z]=heterodyne_trajectory_2ndorder(Jxeff,Jyeff,Jzeff,gamma, timearray, dt, initial_state, Nlength, factormeasured)
%-------------------------------------------------------------------------%
%   Time evolve a quantum trajectory with heterodyne unravelling using the
%   truncated cumulant hierarchy trajectory method up to second order.
%   
%Parameters:
...
sigzres=NaN(Nspins,length(timearray)); sigzres(:,1)=sigz; 

% Set the storage arrays for the cumulants and store the 
% intitial state at t = 0
savequantumcorrs=true;
...
while time<timearray(end)
    ...
    szsx=fzfx+sigz.*sigx';                                                  % Calculate the (z,x)-correlations
    ...
    d_fxfx_corr=2*Jyeff*NBmat.*(-fyfz'.*sigx'-fzfx.*sigy'-sigx.*fyfz-sigy.*fzfx'-sigz.*sigy'.*sigx'-sigz'.*sigy.*sigx)-2*Jzeff*NBmat.*(-fxfy'.*sigz'-fyfz.*sigx'-sigx.*fyfz'-sigz.*fxfy-sigy.*sigz'.*sigx'-sigx.*sigz.*sigy'); 
    ...
    % Wiener noise contributions to the second order cumulants:    
    d_fxfx_stoch = sqrt(gammam/2)*(-2*(sigx.*fxfx) + fzfx).*dWx + sqrt(gammam/2)*(sigy.*fxfx + sigx.*fxfy').*dWy ...
        + sqrt(gammam/2)*(-2*sigx'.*fxfx + fzfx').*dWx' + sqrt(gammam/2)*(sigy'.*fxfx + sigx'.*fxfy).*dWy';

    d_fyfy_stoch = -sqrt(gammam/2)*(sigx.*fyfy + sigy.*fxfy).*dWx + sqrt(gammam/2)*(2*sigy.*fyfy - fyfz').*dWy ...
        -sqrt(gammam/2)*(sigx'.*fyfy + sigy'.*fxfy').*dWx' + sqrt(gammam/2)*(2*sigy'.*fyfy - fyfz).*dWy';
    
    d_fzfz_stoch = -sqrt(gammam/2)*(fzfx' + sigx.*fzfz + sigz.*fzfx').*dWx + sqrt(gammam/2)*(fyfz + sigy.*fzfz + sigz.*fyfz).*dWy ...
        -sqrt(gammam/2)*(fzfx + sigx'.*fzfz + sigz'.*fzfx).*dWx' + sqrt(gammam/2)*(fyfz' + sigy'.*fzfz + sigz'.*fyfz').*dWy';
    
    d_fxfy_stoch =  sqrt(gammam/2)*(-2*(sigx.*fxfy) + fyfz').*dWx + sqrt(gammam/2)*(sigy.*fxfy + sigx.*fyfy).*dWy ...
        -sqrt(gammam/2)*(sigx'.*fxfy + sigy'.*fxfx).*dWx' + sqrt(gammam/2)*(2*sigy'.*fxfy - fzfx').*dWy';
    
    d_fyfz_stoch = -sqrt(gammam/2)*(sigx.*fyfz + sigy.*fzfx').*dWx + sqrt(gammam/2)*(2*sigy.*fyfz - fzfz).*dWy ...
        -sqrt(gammam/2)*(fxfy' + sigx'.*fyfz + sigz'.*fxfy').*dWx' + sqrt(gammam/2)*(fyfy + sigy'.*fyfz + sigz'.*fyfy).*dWy';
    
    d_fzfx_stoch = -sqrt(gammam/2)*(fxfx + sigx.*fzfx + sigz.*fxfx).*dWx + sqrt(gammam/2)*(fxfy' + sigy.*fzfx + sigz.*fxfy').*dWy ...
                   +sqrt(gammam/2)*(-2*sigx'.*fzfx+fzfz).*dWx'+sqrt(gammam/2)*(sigy'.*fzfx+sigx'.*fyfz').*dWy';     
    ...
    fxfx=fxfx+d_fxfx_nonnb*dt+d_fxfx_corr*dt+d_fxfx_ito*dt + d_fxfx_stoch; 
    fxfx=fxfx-diag(diag(fxfx));
    fyfy=fyfy+d_fyfy_nonnb*dt+d_fyfy_corr*dt+d_fyfy_ito*dt + d_fyfy_stoch; 
    fyfy=fyfy-diag(diag(fyfy));
    fzfz=fzfz+d_fzfz_nonnb*dt+d_fzfz_corr*dt+d_fzfz_ito*dt + d_fzfz_stoch; 
    fzfz=fzfz-diag(diag(fzfz));
    fxfy=fxfy+d_fxfy_nonnb*dt+d_fxfy_corr*dt+d_fxfy_ito*dt + d_fxfy_stoch; 
    fxfy=fxfy-diag(diag(fxfy));
    fyfz=fyfz+d_fyfz_nonnb*dt+d_fyfz_corr*dt+d_fyfz_ito*dt + d_fyfz_stoch; 
    fyfz=fyfz-diag(diag(fyfz));
    fzfx=fzfx+d_fzfx_nonnb*dt+d_fzfx_corr*dt+d_fzfx_ito*dt + d_fzfx_stoch; 
    fzfx=fzfx-diag(diag(fzfx));
...
% In this example the results will be preprocessed. Usually, this will be 
% necessary for larger lattices as the memory used to store/save the data 
